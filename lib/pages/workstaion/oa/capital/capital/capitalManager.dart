...
class CapitalManager extends StatefulWidget {


  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return CapitalManagerState();
  }

}

class CapitalManagerState extends State<CapitalManager> {

  bool isEmpty = false;
  List<CapitalApply> lists = [];
  ...
  Future loadMoreData({String beginDate}) async {
    ...
    CapitalListModel capitalListModel = await OAAPI.capitalList(
        start: start, length: pageSize, beginDate: beginDate);
    setState(() {
      lists.addAll(capitalListModel.data);
      totalSize = capitalListModel.total;
      if (lists.length == 0) {
        isEmpty = true;
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: createAppBar(
        "资产领用", automaticallyImplyLeading: true, actions: <Widget>[
        InkWell(
          onTap: () {
            Application.router.navigateTo(context, "${Routes.addCapital}",
  ...
  Widget createItem(CapitalApply capitalModel) {
    String title = capitalModel.title;
          ...
          child: InkWell(
            onTap: () {
              Application.router.navigateTo(
                  context, "${Routes.capitalDetail}?id=${capitalModel.id}",
                  transition: TransitionType.fadeIn);
            },
              ...
              children: <Widget>[
                Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Container(
                      width:230,
                      child: Text("${result[0]}-${capitalModel.createName}",
