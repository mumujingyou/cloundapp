...
class SelectGroup<T> extends StatefulWidget {
  ...
  const SelectGroup(
      ...
      this.padding = const EdgeInsets.all(8.0), this.fontSize})
      : super(key: key);

  @override
  State<StatefulWidget> createState() =>
      SelectGroupState<T>(this.index, listIndex != null ? listIndex : []);
}

class SelectGroupState<T> extends State<SelectGroup<T>> {
  int index;

  List<int> selects;

  SelectGroupState(this.index, this.selects);
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(color: tc,fontSize:widget.fontSize ),
        ),
      ),
    );
  }
  ...
  Widget buildItem(int i) {
    if (widget.style == SelectStyle.circular) {
      if (i == index || selects.contains(i)) {
        return buildItemCircular(i, widget.selectColor, Style.contentColor);
      } else {
        return buildItemCircular(i, widget.unSelectColor, Style.themeColor);
      }
    } else {
      if (i == index || selects.contains(i)) {
        return buildItemRectangle(i, widget.selectColor, Style.contentColor);
      } else {
        return buildItemRectangle(i, widget.unSelectColor, Style.themeColor);
      }
    }
  }
